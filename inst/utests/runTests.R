...
if(require("RUnit", quietly = TRUE)) {
    ...
    if(!(exists("path") && file.exists(path)))
        path <- system.file("utests", package = pkg)

    ## --- Testing ---

    ## Define tests
    source(file.path(path, "suites.R"))

    if(interactive()) {
        cat("Now have RUnit Test Suite 'testAll' for package '", pkg,
            "' :\n", sep='')
        str(testAll)
        cat('', "Consider doing",
        ...
        if( exists( "intergraph.unit.test.output.dir", globalenv() ) ){
			output <- intergraph.unit.test.output.dir
		} else {
